...
for inF in F:
    if inF[-5:]=='.mzML':
        ...
        ouFile.write('\t<note type="input" label="spectrum, path">input-LysC/%s</note>\n'%inF)
        ouFile.write('\t<note type="input" label="output, path">output-LysC/%s</note>\n'%ouF)
        ouFile.write('</bioml>\n')
        ...
        ouFile2.write('cd %s\n\n'%cwd)
        ouFile2.write('tandem.exe input-LysC-xml/%s\n'%ouF)
        ouFile2.close()
        L.append(ouF2)
