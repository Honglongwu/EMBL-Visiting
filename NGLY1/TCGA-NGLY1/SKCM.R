rt = read.table("SKCM__unc.edu__illuminahiseq_rnaseqv2__rsem.genes.results__Jul-08-2014.txt", sep="\t", stringsAsFactors=F)
...
sample = colnames(rt.select)
#platform = rep(c("ga", "hiseq"), times=c(length(colnames(rt.select)),length(colnames(rt2.select))))
platform = rep("hiseq", times=length(colnames(rt.select)))
tumor = rep("SKCM", each=length(sample))
...
count = count.numeric
write.table(annotation$sample,file="SKCM-samples",quote=F,row.names=F,col.names=F)
write.table(count,file="SKCM-count",quote=F,col.names=NA)

###phenotype
###using python to prepare input file
phenotype=read.table("SKCM-samples-phenotype",sep="\t")
...
NGLY1.count[,grepl("TCGA-EE-A2GR-06|TCGA-GN-A266-06|TCGA-FW-A3R5-06|TCGA-EE-A3JA-06",colnames(NGLY1.count))]
...
###
selection = NGLY1.count.t>3000 & annotation$platform=='hiseq'
