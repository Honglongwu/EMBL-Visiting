...
sampleAnnot = sampleAnnot[1:30,]

###mat = assay(geneCounts)

wh = c(seq(1,18),23,24,19,20,21,22,seq(25,30))
...
write.table( res, file=file.path(outfolder, "deNGLY1.nogender.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file=file.path(outfolder, "deNGLY1_sig.nogender.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file=file.path(outfolder, "deNGLY1_sig_proteincoding.nogender.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file=file.path(outfolder, "deNGLY1_sig_proteincoding_up.nogender.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file=file.path(outfolder, "deNGLY1_sig_proteincoding_down.nogender.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file=file.path(outfolder, "deNGLY1_sig_nonproteincoding.nogender.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
...
write.table(cn, file="geneCounts-Human-Normalized.nogender.txt",quote=F,sep="\t",row.names=T,col.names=NA)
save(geneCounts.norm, file = "geneCounts-Human-Normalized.rda")

geneCounts.norm = counts(dds, normalized=F)
cn = cbind.data.frame(ids[match(rownames(geneCounts.norm), ids$gene_id), c("gene_name")],geneCounts.norm)
colnames(cn)[1]='gene_symbol'
write.table(cn, file="geneCounts-Human-raw.nogender.txt",quote=F,sep="\t",row.names=T,col.names=NA)
