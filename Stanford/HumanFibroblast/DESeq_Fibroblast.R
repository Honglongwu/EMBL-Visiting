...
outfolder = file.path(folder, "HumanFibroblast")
if (!file.exists(outfolder))  dir.create(outfolder)

load(file.path(folder, "HumanFibroblast/Counts-Human-Fibroblast.rda"))
sampleAnnot = read.table('SampleAnnot.txt',head=T)
re = '.*(CP1|CP2|CP3|CP4|CP7|FCP1|MCP1).*'
cmp = "Fibroblast"
...
sampleAnnot$cmp = relevel(sampleAnnot$cmp,"control")

mat = assay(geneCounts)
mat = mat[, grepl(re,colnames(geneCounts))]

dds = DESeqDataSetFromMatrix(mat, sampleAnnot, design=~sex+cmp)
dds = DESeq(dds)
