...
write.table( res, file=file.path(outfolder, "deNGLY1-ImmortP5.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig, file=file.path(outfolder, "deNGLY1-ImmortP5_sig.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding, file=file.path(outfolder, "deNGLY1-ImmortP5_sig_proteincoding.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.up, file=file.path(outfolder, "deNGLY1-ImmortP5_sig_proteincoding_up.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.proteincoding.down, file=file.path(outfolder, "deNGLY1-ImmortP5_sig_proteincoding_down.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
write.table( res.sig.nonproteincoding, file=file.path(outfolder, "deNGLY1-ImmortP5_sig_nonproteincoding.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)
...
pdf(file.path(outfolder, "plot_PCA-deNGLY1-ImmortP5.pdf"), width=8, height=6)
print(plotPCA(rld, intgroup=c("sampleStatus","cellType")))
dev.off()

pdf(file.path(outfolder, "plot_MA-deNGLY1-ImmortP5.pdf"), width=8, height=6)
plotMA(results(dds), alpha=0.01)
dev.off()

pdf(file.path(outfolder, "plot_dispEst-deNGLY1-ImmortP5.pdf"), width=8, height=6)
plotDispEsts(dds)
dev.off()
...
write.table(cn, file="geneCounts-Mouse-ImmortP5-Normalized.txt",quote=F,sep="\t",row.names=T,col.names=NA)
save(geneCounts.norm, file = "geneCounts-Mouse-ImmortP5-Normalized.rda")

geneCounts.norm = counts(dds, normalized=F)
cn = cbind.data.frame(ids[match(rownames(geneCounts.norm), ids$gene_id), c("gene_name")],geneCounts.norm)
colnames(cn)[1]='gene_symbol'
write.table(cn, file="geneCounts-Mouse-ImmortP5-raw.txt",quote=F,sep="\t",row.names=T,col.names=NA)
