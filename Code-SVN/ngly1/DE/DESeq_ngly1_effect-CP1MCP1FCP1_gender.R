...
dds = DESeqDataSetFromMatrix(mat, sampleAnnot, design=~individual + gender) #to compare CP2 and CP3 to CP1, unable to use family/gender difference, same comparison here
dds = DESeq(dds)
...
write.table( res, file=file.path(outfolder, "deCP1MCP1FCP1_gender.txt"), quote = FALSE, sep = "\t",  row.names = T, col.names=NA)

rld = rlog(dds, blind=FALSE)
save(dds, rld, res,sampleAnnot,file=file.path(outfolder, "resCP1MCP1FCP1_gender.rda"))

pdf(file.path(outfolder, "plot_PCA-deCP1MCP1FCP1_gender.pdf"), width=8, height=6)
