...
load(file.path(outfolder, "DE_all_NGLY1_DMSO.rda"))

sigplot=function(gene){
plotFolder=file.path(outfolder, "plot")
if (!file.exists(plotFolder))  dir.create(plotFolder)

pdf(file.path(plotFolder, paste0("DE_all_NGLY1_DMSO",paste(gene[1:3],collapse='_'),".pdf")), width=12, height=8)
