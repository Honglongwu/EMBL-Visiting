...
sampleAnnot$sampleStatus = factor(c(rep("control", 4), rep("patient", 12), rep("control", 8)), levels=c("control", "patient"))
sampleAnnot$sampleOrigin = factor(rep(c("cellLine", "patientDerived"), c(4, 20)), levels=c("cellLine", "patientDerived"))
sampleAnnot$family = factor(c(rep("non-w", 4),rep("w", 4), rep("non-w", 8), rep("w", 8)), levels=c("non-w","w"))
...
save(sampleAnnot, file=file.path(folder, "sampleAnnot.rda"))